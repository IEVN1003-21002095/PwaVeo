...
class AuthController:
    ...
    def verificar_contrasena(self, hash_almacenado, password):
        try:
            # Verificar si el hash está en formato bcrypt válido
            if not hash_almacenado or len(hash_almacenado) < 10:
                return False
            return bcrypt.checkpw(password.encode("utf-8"), hash_almacenado.encode("utf-8"))
        except (ValueError, Exception) as e:
            print(f"Error verificando contraseña: {e}")
            # Si el hash es inválido, comparar texto plano (solo para desarrollo)
    ...
    def iniciar_sesion(self, data):
        ...
        return jsonify({
            ...
            "success": True,  # El frontend espera 'success'
            "token": access_token,
